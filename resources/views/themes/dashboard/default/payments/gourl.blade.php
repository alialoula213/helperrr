<div class="row">
    <div class="col-lg-6 col-sm-12 text-center">
        <img src="{{ $invoice->qrcode }}" alt="QrCode" style="width: 300px; height: 300px" class="img-thumbnail" />
    </div>
    <div class="col-lg-6 col-sm-12 text-center">
        <h1>Invoice# {{ $invoice->invoice }}</h1>
        <p>To pay, please send exact amount of {{ setting('currency_code') }} to the given address.</p>
        <h2>{{ crypto_currency($invoice_params->amount, 8) }}</h2>
        <p>≃ {{ fiat_currency($invoice_params->amount * $exchange_rate, setting('fiat_balance_decimals')) }}</p>
        <p class="text-sm">6 confirmations required</p>
        <div class="input-group mb-2">
            <input class="form-control text-center" id="paymentWallet" value="{{ $invoice_params->addr }}" readonly onclick="this.select();">
